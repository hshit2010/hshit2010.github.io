document.addEventListener('DOMContentLoaded', () => {
  ...
  if (form) {
    ...
    // Default Submission: WhatsApp (with Cloudflare Turnstile check)
    form.addEventListener('submit', (e) => {
      e.preventDefault();
      if (!form.checkValidity()) { form.reportValidity(); return; }

      // Turnstile response is injected as a hidden input named "cf-turnstile-response"
      const turnstileEl = document.querySelector('[name="cf-turnstile-response"]');
      const turnstileResponse = turnstileEl ? turnstileEl.value : '';
